    ...
    SonolusApi preprocess() {
        ...
        if (replay) {
            for (var i = 0; i < noteNumber; i++) {
                var index = If(EntityInfoArray[head].archetype == getAid(HoldNote), 16, 15);
	        	if (EntityDataArray[head].generic[index] != 0) combo++;
                else combo = 0;
	        	EntitySharedMemoryArray[head].generic[2] = combo;

	        	maxCombo = Max(maxCombo, combo);
	        	EntitySharedMemoryArray[head].generic[3] = maxCombo;

	        	judgeStatus = Min(judgeStatus, EntityDataArray[head].generic[index]);
	        	EntitySharedMemoryArray[head].generic[4] = judgeStatus;

	        	if (EntityDataArray[head].generic[index] == 2) accscore = accscore + score.perfect;
                else if (EntityDataArray[head].generic[index] == 1) accscore = accscore + score.great;
	        	EntitySharedMemoryArray[head].generic[5] = accscore;

	        	EntitySharedMemoryArray[head].generic[6] = i;
        ...
        var entityCount = 0;
        var lastEntity = EntityInfo.index;
