Bucket NormalDragBucket = defineBucket({
	EngineDataBucketSprite(Sprites.NormalDrag, 0, 0.0, 0.0, 4.0, 2.0, 0)
}, Text.MillisecondUnit);

class DragNote: public Archetype {
	public:

	string name = "Phigros Drag Note";
	bool hasInput = true;
	defineImport(time);
	...
	SonolusApi preprocess() {
		...
		if (hasSFX && autoSFX) PlayScheduled(Clips.Drag, time / levelSpeed, minSFXDistance);
	}

	SonolusApi complete(var hitTime) {
		ExportValue(accuracy, hitTime - time);
		if (Abs(hitTime - time) <= judgment.good) {
			if (hasSFX && !autoSFX) Play(Clips.Drag, minSFXDistance);
	...
	SonolusApi updateSequential() {
		...
		// if (hasSFX && autoSFX && !sfxPlayed && time - times.now <= scheduledSFXTime + Max(0, RuntimeEnvironment.get(2))) PlayScheduled(Clips.Drag, time / levelSpeed, minSFXDistance); sfxPlayed = true;
	...
	SonolusApi touch() {
		...
		if (isFake) return;
		if (times.now < inputTimeMin) return;
		if (hasTouch(info.index)) played = 1;
	}
...
class FakeDragNote: public DragNote {
	...
	string name = "Phigros Fake Drag Note";
	bool hasInput = false;
};